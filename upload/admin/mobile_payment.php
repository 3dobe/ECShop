...
$exc = new exchange($ecs->table('mobile_payment'), $db, 'pay_code', 'pay_name');
...
if ($_REQUEST['act'] == 'list')
{
    /* 查询数据库中启用的支付方式 */
    $pay_list = array();
    $sql = "SELECT * FROM " . $ecs->table('mobile_payment') . " WHERE enabled = '1' ORDER BY pay_order";
    $res = $db->query($sql);
    ...
    /* 取得插件文件中的支付方式 */
    $modules = read_modules('../mobile/includes/modules/payment');
    ...
    $smarty->display('mobile_payment_list.htm');
}
...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    /* 取相应插件信息 */
    $set_modules = true;
    include_once(ROOT_PATH.'mobile/includes/modules/payment/' . $_REQUEST['code'] . '.php');
    ...
    $smarty->assign('action_link',  array('text' => $_LANG['02_payment_list'], 'href' => 'mobile_payment.php?act=list'));
    $smarty->assign('pay', $pay);
    $smarty->display('mobile_payment_edit.htm');
}
...
elseif ($_REQUEST['act'] == 'get_config')
{
    ...
    /* 取相应插件信息 */
    $set_modules = true;
    include_once(ROOT_PATH.'mobile/includes/modules/payment/' . $code . '.php');
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT * FROM " . $ecs->table('mobile_payment') . " WHERE pay_code = '$_REQUEST[code]' AND enabled = '1'";
    $pay = $db->getRow($sql);
    if (empty($pay))
    {
        $links[] = array('text' => $_LANG['back_list'], 'href' => 'mobile_payment.php?act=list');
...
elseif (isset($_POST['Submit']))
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('mobile_payment') .
    ...
    /* 检查是编辑还是安装 */
    $link[] = array('text' => $_LANG['back_list'], 'href' => 'mobile_payment.php?act=list');
    if ($_POST['pay_id'])
    {
        /* 编辑 */
        $sql = "UPDATE " . $ecs->table('mobile_payment') .
    ...
    {
        /* 安装，检查该支付方式是否曾经安装过 */
        $sql = "SELECT COUNT(*) FROM " . $ecs->table('mobile_payment') . " WHERE pay_code = '$_REQUEST[pay_code]'";
        if ($db->GetOne($sql) > 0)
        {
        ...
        else
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('mobile_payment') . " (pay_code, pay_name, pay_desc, pay_config, is_cod, pay_fee, enabled, is_online)" .
